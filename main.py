...
def mine():
    ...
    last_blok = blockchain.last_block
    last_proof = last_blok['proof']
    proof = blockchain.proof_of_work(last_proof)
    # We must receive a reward for finding the proof
    # The sender is "0" to signify that this node has a mined a new coin
    ...
    previous_hash = blockchain.hash(last_blok)
...
def register_nodes():
    ...
    if nodes is None:
        return "Error: Please supply a list of nodes"

    for node in nodes:
        blockchain.register_node(node)

    response = {
        'message': "New noed have been added",
